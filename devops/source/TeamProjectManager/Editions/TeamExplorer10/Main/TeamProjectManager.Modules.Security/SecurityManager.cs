...
namespace TeamProjectManager.Modules.Security
{
    public static class SecurityManager
    {
        ...
        private static string GetAccountName(TeamFoundationIdentity identity)
        {
            ...
            var domain = identity.GetAttribute("Domain", null);
            ...
            else if (!string.IsNullOrEmpty(domain) && !string.IsNullOrEmpty(account))
            {
                // If the identity has a domain and account name, it's a Windows user or group.
                return "{0}\\{1}".FormatInvariant(domain, account);
            }
            else
            {
