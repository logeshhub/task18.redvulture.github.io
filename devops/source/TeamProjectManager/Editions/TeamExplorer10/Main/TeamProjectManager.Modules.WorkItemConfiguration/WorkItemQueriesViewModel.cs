...
namespace TeamProjectManager.Modules.WorkItemConfiguration
{
    ...
    public class WorkItemQueriesViewModel : ViewModelBase
    {
        ...
        // Copied from Team Explorer v12 implementation.
        private static bool IsRootNode(QueryFolder folder)
        {
            if (folder.Project != null)
            {
                QueryHierarchy queryHierarchy = folder.Project.QueryHierarchy;
                if (queryHierarchy.Contains(folder.Id) || (folder == queryHierarchy))
                {
                    return true;
                }
            }
