...
foreach ($workItem in $workItemCollection)
{
    ...
    if( $invalidFields.Count -gt 0 )
    {
        ...
        foreach( $invalidField in $invalidFields )
        {
			...
			#Send-MailMessage -SmtpServer mailserv.mmm.com -To "US-ETFS-Admin@mmm.com" -From "tfs.noreply@mmm.comm" -Subject "WorkItems Validation" -Body "Work Items Validation Output File Attached with this mail. Please Have a look." -Attachments $outputFile
           # Send-MailMessage -SmtpServer mailserv.mmm.com -To "vkumar7@mmm.com" -From "tfs.noreply@mmm.comm" -Subject "WorkItems Validation" -Body "Work Items Validation Output File Attached with this mail. Please Have a look." -Attachments $outputFile
...
#ET: 03/30/16 - Add functionality to EMAIL results to US-ETFS-Admin@mmm.com
# Added by Vishwajeet to send the output as an attachment.
Send-MailMessage -SmtpServer mailserv.mmm.com -To "US-ETFS-Admin@mmm.com" -From "tfs.noreply@mmm.comm" -Subject "WorkItems Validation:$proj" -Body "Work Items Validation Output File Attached with this mail. Please Have a look." -Attachments $outputFile
# Send-MailMessage -SmtpServer mailserv.mmm.com -To "vkumar7@mmm.com" -From "tfs.noreply@mmm.comm" -Subject "WorkItems Validation:$proj"  -Body "Work Items Validation Output File Attached with this mail." -Attachments $outputFile
...
#Below line of code commented out by vishwajeet.
