...
namespace TestCaseManagerCore.BusinessLogic.Managers
{
    ...
    public static class TestSuiteManager
    {
        ...
        /// </returns>
        public static int AddChildSuite(ITestManagementTeamProject testManagementTeamProject, ITestPlan testPlan, int parentSuiteId, string title, out bool canBeAdded, ITestSuiteBase suite = null)
        {
            ...
            if (suite != null && suite is IDynamicTestSuite)
            {
                newSuite = testManagementTeamProject.TestSuites.CreateDynamic();
            }
            else 
            {
                newSuite = testManagementTeamProject.TestSuites.CreateStatic();
            }
            canBeAdded = true;
            ...
            if (suite != null && suite is IDynamicTestSuite)
            {
                IDynamicTestSuite newDynamicSuite = newSuite as IDynamicTestSuite;
                newDynamicSuite.Query = ((IDynamicTestSuite)suite).Query;
            }
            ...
            if (parentSuite != null && parentSuite is IStaticTestSuite && parentSuiteId != -1)
            {
                ...
                parentSuiteStatic.Entries.Add(newSuite);
                log.InfoFormat("Add child suite to suite with Title= {0}, Id = {1}, child suite title= {2}", parentSuite.Title, parentSuite.Id, suite);
            }
            else
            {
                testPlan.RootSuite.Entries.Add(newSuite);
                log.InfoFormat("Add child suite with title= {0} to test plan", suite);
            }
            testPlan.Save();
