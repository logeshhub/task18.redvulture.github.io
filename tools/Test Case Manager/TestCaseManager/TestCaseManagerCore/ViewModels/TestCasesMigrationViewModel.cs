...
namespace TestCaseManagerCore.ViewModels
{
    ...
    public class TestCasesMigrationViewModel : BaseProjectSelectionViewModel
    {
            ...
            foreach (ITestSuiteBase currentSuite in subSuitesCore)
            {
                ...
                try
                {
                    ...
                    if (currentSuite != null)
                    {
                        ...
                        newSuiteId = TestSuiteManager.AddChildSuite(this.destinationTeamProject, this.destinationPreferences.TestPlan, parentId, currentSuite.Title, out canBeAddedNewSuite, currentSuite);
                        if (newSuiteId != 0)
                        {
        ...
        public void MigrateTestCasesFromSourceToDestinationInternal()
        {
            ...
            { 
                ...
                try
                {
                    ...
                    {
                        TestCase newTestCase = currentSourceTestCase.Save(this.destinationTeamProject, this.destinationPreferences.TestPlan, true, null, currentSourceTestCaseTestSteps, true, isMigration: true);
        ...
        public void AddNewTestCasesToNewSuitesDestinationInternal()
        {
            ...
            this.ProgressConcurrentQueue.Enqueue("Populate dynamic test suites...");
            foreach (ITestSuiteBase testSuite in this.destinationPreferences.TestPlan.RootSuite.SubSuites)
            {
                if (testSuite is IDynamicTestSuite)
                {
                    IDynamicTestSuite dynamicTestSuite = testSuite as IDynamicTestSuite;
                    dynamicTestSuite.Repopulate();
                }
            }
        }
